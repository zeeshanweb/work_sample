	...
	<div class="row">
		...
		<div class="seven columns">
			...
			if (class_exists('YdoMigrationProdToStaging'))
			{
			    ...
			    $url = $YdoMigrationProdToStaging->get_replaced_site_url('staging');
			}			
			?>
				<h1>Migrate My Site to Staging</h1>
				<p>The YDO Automated Migration plugin allows you to easily migrate your entire WordPress site from
					PROD to staging.</p>
                <p><!--You staging document root will be: <span style="color:red;"><?php echo $_SERVER['DOCUMENT_ROOT'].'/staging';?></span><br />-->
                You staging url will be: <span style="color:red;"><?php echo $url;?></span></p>
				<p>Please enter the values into the fields below, and click "Migrate".</p>

                <?php echo wp_nonce_field( 'prod_to_staging_action', 'prod_to_staging_name' ) ?>
				<div class="row">
				...
				<div class="row">
					...
					<hr/>
						<?php //submit_button(); ?>
                        <input type='submit' id='migratesubmit' value='Copy production to staging' class="button button-primary">
			</form>
		</div>
		</div>
